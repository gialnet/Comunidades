...
import es.redmoon.comunidades.comuneros.*;
 ...
 * @author antonio
 */
public class AjaxRegador extends HttpServlet {
            ...
            case "getRiegosPendientesByPool":
                {
                    ...
                    List<TuplasVw_pendiente_riego> myListaPendiente = 
                            myWaterer.getRiegosPendientesByPool();
                    response.getWriter().write(gson.toJson(myListaPendiente));
                    break;
                }
            case "getListaRiegosPendientes":
                {
                    ...
                    //System.out.print(xIDPoliza);
                    List<TuplasVw_pendiente_riego> myListaPendiente = 
                            myWaterer.getListaRiegosPendientes(Integer.parseInt(pagina),Integer.parseInt(size));
                    response.getWriter().write(gson.toJson(myListaPendiente));
                    break;
                }
            default:
                response.getWriter().write("Error, mensaje no contemplado en Regador: "+accion);
                break;
        }
    }
